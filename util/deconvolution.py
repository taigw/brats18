...
def infer_output_dims_tensor(input_dims, strides, kernel_sizes, padding):
    """
        infer output dims from tensor,
        the dim can be different in different directions.
        Note: dilation is not considerted here.
        input_dims: a tensor of [d, h, w]
    """
    if padding == 'VALID':
        output_dims = input_dims * tf.constant(strides)
        offset = [max(kernel_sizes[i] - strides[i], 0)
                    for i in range(len(kernel_sizes))]
        output_dims = output_dims + tf.constant(offset)
    else:
        output_dims = input_dims * tf.constant(strides)
    return output_dims
...
class DeconvolutionalLayer(TrainableLayer):
    ...
    def layer_op(self, input_tensor, is_training=None, keep_prob=None, input_image_shape = None):
        # init sub-layers
        deconv_layer = DeconvLayer(n_output_chns=self.n_output_chns,
                                   ...
                                   name='deconv_')
        output_tensor = deconv_layer(input_tensor, input_image_shape = input_image_shape)
