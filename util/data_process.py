...
def get_largest_two_components(img, threshold = 2000):
    """
    ...
    if(len(sizes) == 0):
        return output, component_list
    max_size = sizes.max()
    sizes_list = [sizes[i] for i in range(len(sizes)) if ((sizes[i]/max_size) > 0.1 or sizes[i] > threshold)]
    sizes_list.sort()
    if(len(sizes_list) > 2):
        sizes_list = sizes_list[-2:]
    for temp_size in sizes_list:
        temp_lab  = np.where(sizes == temp_size)[0] + 1
        temp_component = np.asarray(labeled_array == temp_lab, np.uint8)
        output = output + temp_component
